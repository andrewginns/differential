...
def deduplicate_content_registry(registry_path: str) -> Tuple[int, List[str]]:
    ...
    for file_path, content_ids in path_to_ids.items():
        if len(content_ids) > 1:
            ...
            logger.info(
                f"Found {len(content_ids)} IDs for path {file_path}. Keeping {content_ids[0]}, removing {content_ids[1:]}"
            )
        else:
...
def build_registry(
    data_dir: str, registry_path: str, file_extensions: List[str] = None
) -> Tuple[int, List[str]]:
...
def main():
    ...
    common_parser.add_argument(
        "--data-dir", default="data", help="Base directory for content files"
    )

    # Build command
    build_parser = subparsers.add_parser(
        "build", parents=[common_parser], help="Build the content registry from scratch"
    )
    ...
    validate_parser = subparsers.add_parser(
        "validate", parents=[common_parser], help="Validate the content registry"
    )
    ...
    clean_parser = subparsers.add_parser(
        "clean", parents=[common_parser], help="Clean up the content registry"
    )
